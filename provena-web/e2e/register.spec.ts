...
// The cookie-consent banner is fixed to the bottom of the viewport and can sit
// over the form's submit button, so dismiss it first (as a real user would).
async function acceptCookies(page: Page) {
  const accept = page.getByRole('button', { name: /accept all/i })
  if (await accept.isVisible().catch(() => false)) {
    await accept.click()
    await accept.waitFor({ state: 'hidden' }).catch(() => {})
  }
}
