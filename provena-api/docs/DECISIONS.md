...
- **Advisory discount preview (#197).** A buyer-facing `POST /discounts/validate/` checks a code against the buyer's current cart and reserves nothing (`place_order` re-validates authoritatively, so a code can still lapse before checkout). It returns the discount amount and the full previewed `new_total` from the **same** `compute_order_pricing` pass used at checkout (`goods − discount + shipping`, VAT-inclusive), so the quoted saving and total match what is charged rather than a raw goods discount. Platform-brokered delivery uses the supplier's standing fee in the preview, since the live courier quote needs the delivery address and is only resolved at checkout.
...
- **Admin item refund (#213).** An admin can refund selected order items directly (`POST /orders/admin/{reference}/refund-items/`). The chosen items are grouped by sub-order (one supplier each) and each group is driven through the same refund path as a return, so the buyer is refunded the selected units' value and **each supplier's payout is reversed for the items they sold** - closing the gap where the earlier amount-only admin refund (`payments.services.initiate_refund`) never clawed back a payout. Unlike a buyer return it is not gated on delivery status or the return window, and the refunded units restock. The amount-only `AdminRefundView` remains for platform-absorbed goodwill refunds (no payout reversal).
...
**Decision:**
1. **A `return_policy` enum** covering the default plus the two UK CCR-2013 exemption classes: `RETURNABLE` (standard 14-day change-of-mind return); `DEFECTIVE_ONLY` (perishable/exempt goods, reg 28(3)(c): no change-of-mind return, defects go via a dispute); and `SEALED` (hygiene/sealed goods, reg 28(3)(b): returnable *only while unopened*). Because the platform cannot observe whether a seal is broken, `SEALED` is treated as returnable at the request gate (`is_returnable` is true for `RETURNABLE` and `SEALED`) with an "only if unopened" condition surfaced to the buyer; the supplier verifies the seal on inspection via the existing approve/reject step. Only `DEFECTIVE_ONLY` hard-blocks a change-of-mind return.
...
- The sealed/hygiene exemption class (`SEALED`) and the buyer-facing returns-policy explainer on the product page were both completed as part of this ADR. No return-eligibility items remain deferred.
